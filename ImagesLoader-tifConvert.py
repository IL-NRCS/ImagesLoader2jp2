...
## Date modified: March 07, 2023             ##
###############################################

## This script creates a new directory of .tif images
## from an existing directory of .tif images. It also creates a
## mosaic file that groups all the .tif images together.
## 
...
def georeference(Target_shapefile,raster_file,path_field,flt_dir_field,scale_dir_field):
    ...
    if sep not in path_field :
        sep='\\'
	
    raster_name=raster_file[raster_file.rfind(sep)+1:]
    ...
    if os.path.exists(Target_shapefile):
        ...
        if res=='':
            ...
            Output_location= raster_file[:raster_file.rfind(sep)]
                        ...
                        if FL_DIR is not None and str(FL_DIR)!='':
                            ...
                            if FL_DIR == 'NS':
                                if str(SCALE) == '10000':
                                    ...
                                    longitude = LONG - 1600
                                    latitude = LAT + 1600
                                    f = open(Output_location + sep + raster_name + '.tfw', 'w')
...
def reducedPathCreate(image_path,reducedFolderName):
    ...
    newPathFile=path+'\\'+image_name+'.tif'
    return newPathFile
def rasSize(raster):
...
def copyFromToReduce(fromPath,toPath):
    arcpy.management.CopyRaster(fromPath,toPath, '', None, "256", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF", "NONE", "CURRENT_SLICE", "NO_TRANSPOSE")
    ...
    for key, value in groupby(list_images, key_func):
        ...
        for imPath in list_dic_im :
            ...
            try :
                ...
                arcpy.AddMessage(gdbname+'/'+mdname+nb_images)
                arcpy.management.AddRastersToMosaicDataset(gdbname+'/'+mdname+nb_images, "Raster Dataset", imPath['file'], "UPDATE_CELL_SIZES", "UPDATE_BOUNDARY", "NO_OVERVIEWS", None, 0, 1500, None, '', "SUBFOLDERS", "ALLOW_DUPLICATES", "BUILD_PYRAMIDS", "CALCULATE_STATISTICS", "NO_THUMBNAILS", '', "NO_FORCE_SPATIAL_REFERENCE", "ESTIMATE_STATISTICS", None, "NO_PIXEL_CACHE")
